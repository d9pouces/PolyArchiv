...
for line in codecs.open(
    os.path.join("polyarchiv", "__init__.py"), "r", encoding="utf-8"
):
...
if os.path.isfile(engines_file):
    ...
    if parser.has_section("backup_points"):
        backup_points = [
            "%s = %s" % (key, value) for key, value in parser.items("backup_points")
        ]
    if parser.has_section("collect_points"):
        collect_points = [
            "%s = %s" % (key, value) for key, value in parser.items("collect_points")
        ]
...
setup(
    ...
    entry_points={
        "console_scripts": ["polyarchiv%s = polyarchiv.cli:main" % command_suffix],
        "polyarchiv.sources": sources,
        "polyarchiv.backup_points": backup_points,
        "polyarchiv.collect_points": collect_points,
