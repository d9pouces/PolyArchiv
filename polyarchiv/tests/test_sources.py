...
log = {
    ...
    "loggers": {
        "polyarchiv": {"handlers": ["stream"], "level": "DEBUG", "propagate": False}
    },
}
...
class TestSources(FileTestCase):
    ...
    def setUp(self):
        ...
        self.collect_point = FileRepository(
            "test_repo",
            local_path=self.collect_point_path,
    ...
    def test_local_files(self):
        source = LocalFiles(
            "local_files",
            self.collect_point,
            destination_path="local_files",
            source_path=self.original_dir_path,
        )
        source.backup()
        self.assertEqualPaths(
            self.original_dir_path,
            os.path.join(self.collect_point.export_data_path, "local_files"),
        )
    ...
    def test_remote_files(self):
        ...
        self.assertEqualPaths(
            original_dir_path,
            os.path.join(self.collect_point.export_data_path, "remote_files"),
        )
    ...
    def test_read_ldif(self):
        ...
        self.assertEqual(
            "/var/lib/ldap", Ldap.get_database_folder(io.BytesIO(content), "1")
        )

    def test_ldap(self):
        filename = "ldap.ldif"
        source = Ldap(
            "ldap", self.collect_point, destination_path=filename, use_sudo=True
        )
        source.backup()
